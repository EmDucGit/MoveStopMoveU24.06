...
public class Character : Player
{
    ...
    void CheckJoyStick()
    {
        ...
        {
            isMoving = true;
            anim.SetTrigger(CacheString.animRunning);
        ...
        } else
        {
            ...
            if(!enemyOnRange)
            {
                anim.SetTrigger(CacheString.animIdle);
            }
        }
    }
    ...
    void Update()
    {
        if(isDead)
        {
            anim.SetTrigger(CacheString.animDead);
            return;
        }
        CheckJoyStick();
        if (isMoving)
        ...
        {
            anim.SetTrigger(CacheString.animAttack);
        }
    }
    ...
    {
        if (other.CompareTag(CacheString.bullet))
        {
            isDead = true;
            //ref
